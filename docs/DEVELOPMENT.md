...
稳定基线是 3.7.14（沿用 3.7.1 起的场景预设打开行为）。不要重新引入已放弃的 3.8.0 预设合并导入实验。
...
`mh_fit.py` 负责 UV 对应、编号修复和配件适配。内嵌临时 Maya 命令负责 API 点位修改的撤销；修改其实现时必须升级命令名和缓存文件版本，禁止运行时卸载仍在撤销队列中的旧命令。
